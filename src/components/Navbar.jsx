      ...
      if (searchQuery.trim().length >= 2) {
        ...
        try {
          const response = await fetch(
            `https://propix8.com/api/search?q=${searchQuery}`,
          );
          const result = await response.json();
          if (result.status) {
          ...
          searchResults.map((unit) => (
            ...
            >
              ...
              <div className="overflow-hidden">
                ...
                <p className="text-xs font-bold text-[#3E5879] mt-1">
  ...
  return (
    <div className="flex flex-col w-full fixed top-0 z-9999 backdrop-blur-md bg-white/80 shadow-sm transition-all duration-300">
      ...
      >
        <div className="w-full max-w-7xl flex justify-between items-center gap-2">
          <div className="flex items-center gap-3">
            ...
            <div className="lg:hidden flex items-center gap-1">
              <button
                ...
                className="p-2 text-[#3E5879] hover:bg-gray-100 rounded-lg"
              >
          ...
          <div className="hidden lg:flex items-center gap-6 text-[#000000] font-bold">
            <NavLink
              to="/"
              className={({ isActive }) =>
                isActive ? "text-[#3E5879]" : "hover:text-[#3E5879]"
              }
            >
              الرئيسية
            </NavLink>
          ...
          {/* Desktop Search */}
          <div
            className="hidden lg:flex relative flex-1 max-w-sm mx-4"
            ref={searchRef}
          >
          ...
          <div className="flex items-center gap-2">
            ...
            {userToken ? (
                ...
                {isOpen && (
                  <div className="absolute left-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border border-gray-100 py-2 z-[999] overflow-hidden">
                    ...
                    <Link
                      ...
                      className="flex items-center gap-3 px-4 py-3 text-gray-600 hover:bg-[#3E5879]/5 transition-colors"
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-3 px-4 py-3 text-[#dc2626] hover:bg-red-50 border-t border-gray-100 mt-1 font-bold transition-colors"
                    >
                      <LogOut size={18} />{" "}
              ...
              <Link
                to="/signin"
                className="border-2 border-[#3E5879] text-[#3E5879] px-4 md:px-6 py-1.5 rounded-xl font-bold hover:bg-[#3E5879] hover:text-white transition-all text-sm"
              >
                دخول
              </Link>
            )}
        ...
        {isMobileMenuOpen && (
          <div
            className="lg:hidden absolute top-full right-0 w-full bg-white border-b shadow-xl py-6 flex flex-col items-center gap-5 z-[998] animate-in slide-in-from-top duration-300"
            ref={mobileMenuRef}
          >
            <NavLink
              to="/"
              onClick={() => setIsMobileMenuOpen(false)}
              className="font-bold text-gray-700 hover:text-[#3E5879]"
            >
              الرئيسية
            </NavLink>
            <NavLink
      ...
      {showMobileSearch && (
        <div
          className="lg:hidden bg-white px-4 py-3 border-b border-gray-100 animate-in slide-in-from-top duration-300 shadow-inner"
          ref={mobileSearchRef}
        >
