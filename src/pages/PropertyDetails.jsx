...
import {
  ...
  Maximize, Phone, Play, ChevronRight, Loader2, X, Send, User, AtSign, Smartphone, MessageSquareMore, CalendarCheck, ChevronLeft
} from 'lucide-react';
...
export default function PropertyDetails() {
  ...
  const toggleFavorite = async () => {
    const token = localStorage.getItem('userToken'); 
    if (!token) {
      toast.warning("يرجى تسجيل الدخول أولاً لاستخدام المفضلة", { position: "top-center" });
      return;
    }
  ...
  const handleSubmitReview = async (e) => {
    ...
    const token = localStorage.getItem('userToken');
    if (!token) {
      toast.warning("يرجى تسجيل الدخول أولاً لإضافة تقييم", { position: "top-center" });
      return;
    }
    if (rating === 0) {
    ...
    <div className="bg-white rounded-xl w-full max-w-2xl max-h-[95vh] overflow-y-auto shadow-2xl animate-in fade-in zoom-in duration-300">
      ...
      <div className="p-5">
        ...
        <div className="flex flex-row justify-between items-center mb-4 gap-3 bg-gray-50 p-3 rounded-lg">
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <h4 className="text-sm font-bold text-gray-800">{unit.owner?.name}</h4>
              <div className="flex items-center gap-2 text-gray-500 mt-0.5">
                <span className="text-xs font-medium">{unit.owner?.phone || "010000000"}</span>
                <Phone size={12} className="text-[#3E5879] rotate-[270deg]" />
              </div>
            </div>
          </div>
          <button className="text-[#3E5879] font-bold text-xs hover:underline">التفاصيل</button>
        </div>
        ...
        <form onSubmit={handleSendMessage} className="space-y-4">
          ...
          >
            {submitting ? <Loader2 className="animate-spin mx-auto" size={20} /> : "إرسال الرسالة"}
          </button>
        </form>
      </div>
    </div>
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-10 mt-12">
        <div className="lg:col-span-2 space-y-10">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 text-sm">
              ...
              <div className="flex items-center gap-3 p-3 border-b border-gray-50">
                ...
                <button 
                  onClick={() => unit.compound?.id && navigate(`/compoundUnits/${unit.compound.id}`)}
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
            <h3 className="text-xl font-bold text-[#3E5879] mb-6 border-r-4 border-[#3E5879] pr-3 uppercase tracking-wide">أضف تقييمك</h3>
            <form onSubmit={handleSubmitReview} className="space-y-4">
              <div className="flex gap-2 mb-4">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button key={star} type="button" onClick={() => setRating(star)} className="transition-transform hover:scale-125">
                    <Star size={32} fill={star <= rating ? "#FFD700" : "none"} color={star <= rating ? "#FFD700" : "#D1D5DB"} />
                  </button>
                ))}
              </div>
              <textarea
                ...
                required
              ></textarea>
              <button disabled={submittingReview} type="submit" className="bg-[#3E5879] text-white px-10 py-4 rounded-2xl font-black flex items-center gap-3 hover:bg-[#2c3d54] disabled:opacity-50 transition-all shadow-md active:scale-95">
                {submittingReview ? <Loader2 className="animate-spin" /> : "نشر التقييم"}
              </button>
            </form>
          </div>

          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
            <h3 className="text-xl font-bold text-[#3E5879] mb-8 border-r-4 border-[#3E5879] pr-3 uppercase tracking-wide">تقييمات الزوار</h3>
            {reviewsLoading ? (
              <div className="flex justify-center py-10"><Loader2 className="animate-spin text-[#3E5879]" /></div>
            ) : unitReviews.length > 0 ? (
              <div className="space-y-6">
                {unitReviews.map((rev) => (
                  <div key={rev.id} className="p-2 bg-gray-50 rounded-[1rem] border border-gray-100">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 bg-[#3E5879] text-white rounded-full flex items-center justify-center font-black">{rev.user?.name?.charAt(0)}</div>
                        <div>
                          <h4 className="font-bold text-[#3E5879]">{rev.user?.name}</h4>
                          <span className="text-[10px] text-gray-400">{new Date(rev.created_at).toLocaleDateString('ar-EG')}</span>
                        </div>
                      </div>
                ...
                {reviewsPagination.last_page > 1 && (
                  <div className="flex justify-center items-center gap-4 mt-8">
                    <button 
                      ...
                      onClick={() => fetchReviews(reviewsPagination.current_page - 1)}
                      className="p-3 rounded-xl bg-white border border-gray-200 disabled:opacity-30 hover:bg-gray-50 transition-all"
                    >
                      <ChevronRight size={20} />
                    </button>
                    <span className="font-black text-[#3E5879]">صفحة {reviewsPagination.current_page} من {reviewsPagination.last_page}</span>
                    <button 
              ...
              <p className="text-center text-gray-400 font-bold py-10">لا توجد تقييمات لهذا العقار بعد.</p>
            )}
          </div>
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
            ...
            <div className="rounded-[2.5rem] overflow-hidden h-[400px] bg-gray-100 border-4 border-white shadow-lg relative">
              {unit.latitude && unit.longitude ? (
                <iframe
                  width="100%" height="100%" frameBorder="0" scrolling="no" marginHeight="0" marginWidth="0" title="Property Location"
          ...
          <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-gray-50 sticky top-24">
            ...
            {unit.developer && (
              <div className="mt-10 pt-8 border-t border-gray-50">
                ...
                <div 
                  onClick={() => unit.developer?.id && navigate(`/compoundUnits/${unit.developer.id}`)}
                ...
                >
                  ...
                  <span className="text-sm font-black text-gray-500 group-hover:text-[#3E5879] transition-colors underline decoration-dotted decoration-2 underline-offset-4">{unit.developer.name}</span>
                </div>
              </div>
            )}
          </div>
            ...
            {relatedUnits.slice(0, 3).map((item) => (
              ...
              >
                <div className="relative h-64 overflow-hidden">
                  <img src={item.main_image} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt={item.title} />
