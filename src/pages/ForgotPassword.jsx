...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        // رسالة نجاح
        toast.success("تم إرسال رمز استعادة كلمة المرور بنجاح!", {
          position: "top-right",
          autoClose: 3000,
        });
        ...
        toast.error(result.message || "البريد الإلكتروني غير صحيح أو غير مسجل", {
          position: "top-right",
        });
      }
    } catch (error) {
    ...
    <div className="h-screen w-full flex bg-[#ECEFF3] font-cairo overflow-hidden" dir="rtl">
      ...
      <div className="hidden lg:block lg:w-1/2 h-full p-4">
        <div className="relative h-full w-full">
          <img src={signupImg} alt="Real Estate" className="w-full h-full object-cover rounded-[2rem] shadow-sm" />
      ...
      <div className="w-full lg:w-1/2 h-full flex flex-col justify-center px-8 md:px-24">
        <div className="max-w-md w-full mx-auto text-center">
          <h2 className="text-[#3E5879] text-4xl font-bold mb-3">نسيت كلمة المرور؟</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              {loading ? <Loader2 className="animate-spin" /> : "إرسال طلب الاستعادة"}
            </button>
          </form>

          <Link to="/signin" className="mt-8 inline-flex items-center gap-2 text-[#3E5879] font-bold hover:underline">
