...
export default function ProfileInfo() {
  ...
  if (loading) return <div className="flex justify-center py-20"><Loader2 className="animate-spin text-[#3E5879]" size={40} /></div>;

  const defaultAvatar = `https://ui-avatars.com/api/?name=${user?.name}&background=3E5879&color=fff&bold=true`;

  return (
    <div className="bg-[#F8F9FA] min-h-[600px] p-4 md:p-8 font-cairo" dir="rtl">
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-[2rem] p-6 md:p-10 shadow-sm border border-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <ReadOnlyField label="نوع المستخدم" value={user?.role === 'seller' ? 'بائع' : 'مشتري'} />
      ...
      {isEditModalOpen && (
        <div className="mt-20  fixed inset-0 z-[999] flex items-center justify-center bg-black/40 backdrop-blur-[2px] p-4">
          <div className="bg-white w-full max-w-lg rounded-[1rem] shadow-2xl relative overflow-hidden animate-in zoom-in duration-200">

            <div className="p-4 border-b border-gray-50 flex justify-between items-center bg-[#F8F9FA]">
              <h3 className="text-lg font-black text-[#3E5879]">تعديل البيانات الشخصية</h3>
              <button onClick={() => setIsEditModalOpen(false)} className="p-1 hover:bg-red-50 rounded-full text-gray-400 hover:text-red-500 transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-3 md:p-8 max-h-[75vh] overflow-y-auto custom-scrollbar">
              ...
              <form className="space-y-4" onSubmit={handleUpdateProfile}>
                <div className="grid grid-cols-1 gap-4">
                  <InputField label="الاسم بالكامل" name="name" value={formData.name} onChange={handleInputChange} />
                </div>

                <InputField label="رقم الهاتف" name="phone" value={formData.phone} onChange={handleInputChange} type="tel" />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1 text-right">
                    <label className="text-[11px] font-bold text-gray-500 pr-1">المدينة</label>
                    <select name="city_id" value={formData.city_id} onChange={handleInputChange} className="w-full p-2.5 bg-[#F9FAFB] border border-gray-200 rounded-lg outline-none text-right text-xs appearance-none cursor-pointer">
                      {cities.map(city => <option key={city.id} value={city.id}>{city.name}</option>)}
                    </select>
                  </div>
                  <SelectField label="نوع المستخدم" value={user?.role === 'seller' ? 'بائع' : 'مشتري'} />
                </div>

                <button type="submit" disabled={submitting} className="w-full bg-[#3E5879] text-white font-black py-3 rounded-xl mt-4 shadow-lg hover:brightness-110 transition-all flex justify-center items-center">
                  {submitting ? <Loader2 className="animate-spin mr-2" size={18} /> : 'حفظ'}
                </button>
              </form>
            </div>
          </div>
...
function ReadOnlyField({ label, value }) {
  return (
    <div className="space-y-1.5 text-right">
      <label className="block text-gray-400 text-xs font-bold pr-1">{label}</label>
      <div className="w-full p-3.5 bg-[#F4F6F8] rounded-xl border border-gray-50 text-[#3E5879] font-bold text-sm shadow-sm">{value || "---"}</div>
    </div>
  );
}
...
function InputField({ label, name, value, onChange, placeholder, type = "text" }) {
  return (
    <div className="space-y-1 text-right w-full">
      ...
      <input type={type} name={name} value={value} onChange={onChange} placeholder={placeholder} className="w-full p-2.5 bg-[#F9FAFB] border border-gray-200 rounded-lg outline-none text-right placeholder-gray-300 text-xs focus:border-[#3E5879] transition-all" />
    </div>
  );
}

function PasswordField({ label, name, value, onChange, show, toggle, isConfirm = false, required = false }) {
  return (
    <div className="space-y-1 text-right">
      <label className="text-[11px] font-bold text-gray-500 pr-1">{label} {required && "*"}</label>
      <div className="relative">
        <input
          name={name}
          value={value}
        ...
        />
        {isConfirm && <Lock className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-300" size={14} />}
        <button type="button" onClick={toggle} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-300 hover:text-gray-500">
...
function SelectField({ label, value }) {
  return (
    <div className="space-y-1 text-right">
      <label className="text-[11px] font-bold text-gray-500 pr-1">{label}</label>
      <select disabled className="w-full p-2.5 bg-[#F4F6F8] border border-gray-200 rounded-lg outline-none text-right text-xs appearance-none opacity-70">
        <option>{value}</option>
      </select>
    </div>
  );
