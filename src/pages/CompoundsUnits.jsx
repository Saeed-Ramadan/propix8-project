...
import { BedDouble, Bath, Maximize, MapPin, Tag, ArrowLeft, Loader2, ChevronRight, ChevronLeft, Building2, Info } from 'lucide-react';
...
function CompoundsUnits() {
    ...
    useEffect(() => {
        const fetchCompound = async () => {
            try {
                const response = await axios.get(`https://propix8.com/api/compounds/${id}`);
    ...
    if (loading) return (
        <div className="flex flex-col justify-center items-center h-screen bg-gray-50" dir="rtl">
            <Loader2 className="w-12 h-12 text-[#3E5879] animate-spin mb-4" />
            <p className="text-gray-500 font-bold font-cairo">جاري تحميل بيانات الكومباوند...</p>
        </div>
    );

    if (!compoundData) return <div className="text-center py-20 font-bold text-red-500 font-cairo">لم يتم العثور على الكومباوند!</div>;

    // حسابات الـ Pagination
    const indexOfLastUnit = currentPage * unitsPerPage;
    const indexOfFirstUnit = indexOfLastUnit - unitsPerPage;
    const currentUnits = compoundData.units.slice(indexOfFirstUnit, indexOfLastUnit);
                    ...
                    <div className="flex flex-col items-center text-center">
                        ...
                        <div className="max-w-2xl bg-white/10 backdrop-blur-md p-6 rounded-[2.5rem] border border-white/10 shadow-xl">
                            ...
                            <p className="text-white text-lg md:text-xl leading-relaxed font-medium">
                                {compoundData.description || "تجربة سكنية متكاملة تجمع بين الفخامة والخصوصية في قلب أرقى المناطق الحيوية."}
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    {currentUnits.map((unit) => (
                        ...
                        >
                            ...
                            <div className="relative h-72 bg-gray-200 overflow-hidden">
                                <img
                                    src={unit.main_image || "https://images.unsplash.com/photo-1512917774080-9991f1c4c750?q=80&w=2070&auto=format&fit=crop"}
                                    alt={unit.title}
                                ...
                                <div className="absolute bottom-6 right-6">
                                     <div className="bg-[#3E5879] text-white px-5 py-2.5 rounded-2xl shadow-2xl font-black text-xl border border-white/20">
                                        {parseFloat(unit.price).toLocaleString()} <span className="text-xs font-normal opacity-80 mr-1">ج.م</span>
                                     </div>
                                </div>
                            </div>
                            ...
                            <div className="p-8 flex-grow flex flex-col">
                                ...
                                <div className="grid grid-cols-3 gap-3 py-6 border-y border-gray-50 mb-6">
                                    <div className="flex flex-col items-center p-3 rounded-[1.5rem] bg-gray-50 group-hover:bg-blue-50 transition-colors">
                                        ...
                                        <span className="text-xs font-black text-gray-700">{unit.rooms} غرف</span>
                                    </div>
                                    <div className="flex flex-col items-center p-3 rounded-[1.5rem] bg-gray-50 group-hover:bg-blue-50 transition-colors">
                                        <Bath size={24} className="text-[#3E5879] mb-1" />
                                        <span className="text-xs font-black text-gray-700">{unit.bathrooms} حمام</span>
                                    </div>
                                    <div className="flex flex-col items-center p-3 rounded-[1.5rem] bg-gray-50 group-hover:bg-blue-50 transition-colors">
                                        <Maximize size={24} className="text-[#3E5879] mb-1" />
                                        <span className="text-xs font-black text-gray-700">{parseInt(unit.area)} م²</span>
                                    </div>
                                </div>
